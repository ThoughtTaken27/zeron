...
| Aside CLI adapter | done (bounded) | macOS-first native adapter over the public `aside mcp` JSON-RPC surface; dynamic model catalog imported per `models()` call from the signed-in Aside config (`aside account list` + `~/.aside/u/{n}/models.json`, `defaultModel` ordering only, static `Default`/`Fast` fallback, failures never cached; secrets never read/logged), `off` through `max` plus `ultrabrowse` effort choices, permission/host forwarding, `--account` routing with per-run validation, `-m provider/model` slash routing, resume, known-session in-turn steer, stop, and explicit remote-host support. MCP returns completed results rather than a stream of tool events, so Zeron does not promise streaming tool events or token deltas. Requires the Aside app/daemon/keychain environment; `ASIDE_EXECUTABLE` overrides CLI resolution. iOS untouched (wire delivers live catalog; fallback stays default/fast). |
