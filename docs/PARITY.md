...
| Aside CLI adapter | done (bounded) | macOS-first native adapter over the public `aside mcp` JSON-RPC surface; dynamic model catalog imported per `models()` call from the signed-in Aside config (`aside account list` + `~/.aside/u/{n}/models.json`, `defaultModel` ordering only, static `Default`/`Fast` fallback, failures never cached; secrets never read/logged), single `effort` picker knob (`default`/`off`/`minimal`/`low`/`medium`/`high`/`xhigh`/`max`/`ultrabrowse`, `default` omits the flag so Aside decides; no Reasoning ladder — stored reasoning levels are ignored, never an error), permission/host forwarding, `--account` routing with per-run validation, `-m provider/model` slash routing, resume via the session id parsed from the exec result's first text line (`SessionStarted` first so the engine records it; loud error instead of a silent new session when no id exists), known-session in-turn steer, stop, and explicit remote-host support. MCP returns completed results rather than a stream of tool events, so Zeron does not promise streaming tool events or token deltas. Requires the Aside app/daemon/keychain environment; `ASIDE_EXECUTABLE` overrides CLI resolution. iOS fallback mirrors the Rust rows (`default`/`fast`, no ladder). |
